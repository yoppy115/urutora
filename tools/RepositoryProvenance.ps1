...
function Get-RepositoryProvenance {
    ...
    $repositoryArguments = @('-c', "safe.directory=$resolvedRoot", '-C', $resolvedRoot)
    $commit = (& $gitCommand.Source @repositoryArguments rev-parse HEAD).Trim()
    if ($LASTEXITCODE -ne 0 -or $commit -notmatch '^[0-9a-fA-F]{40}$') {
        throw "Unable to resolve a full Git commit for $resolvedRoot."
    }

    $status = @(& $gitCommand.Source @repositoryArguments status --porcelain=v1 --untracked-files=normal)
