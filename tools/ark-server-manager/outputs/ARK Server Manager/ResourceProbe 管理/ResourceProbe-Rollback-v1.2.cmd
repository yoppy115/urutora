@echo off
chcp 65001 >nul
title ResourceProbe v1.2へロールバック
...
set "BACKUP=%~dp0Backups\2026-08-11_ResourceProbe-v1.2"
set "PLUGIN=D:\arkserver\ShooterGame\Binaries\Win64\ArkApi\Plugins\ResourceProbe"

if not exist "%BACKUP%\ResourceProbe v1.2.dll" (
  echo.
  echo v1.2バックアップが見つかりません。
  echo %BACKUP%
  echo.
  pause
  exit /b 1
...
copy /Y "%BACKUP%\ARK Server Manager v1.12.2.exe" "%~dp0..\ARK Server Manager.exe" >nul
copy /Y "%BACKUP%\ResourceProbe v1.2.dll" "%PLUGIN%\ResourceProbe.dll" >nul
copy /Y "%BACKUP%\PluginInfo v1.2.json" "%PLUGIN%\PluginInfo.json" >nul
if exist "%PLUGIN%\ResourceProbe.dll.disabled" del /Q "%PLUGIN%\ResourceProbe.dll.disabled"

echo.
echo ARK Server Manager v1.12.2 / ResourceProbe v1.2へ戻しました。
echo.
pause
