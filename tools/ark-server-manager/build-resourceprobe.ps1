param(
    [string]$OutputPath = (Join-Path $PSScriptRoot 'build\ResourceProbe.dll')
)
...
$source = Join-Path $PSScriptRoot 'src\ResourceProbe\ResourceProbe.cpp'
...
$objectPath = Join-Path (Split-Path -Parent $OutputPath) 'ResourceProbe.obj'
...
if ($LASTEXITCODE -ne 0) { throw 'ResourceProbe build failed.' }
