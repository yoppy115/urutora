    ...
    private static void WorldSessionsAreNumberedAndLogged()
    {
        ...
        try
        {
            ...
            using (var first = store.CreateNextWorld(simulationConfig, SimulationConfigPath(), 9000))
            {
                ...
                Equal("v0.2.6", first.Info.ReleaseVersion);
                Equal("v0.2.6", Directory.GetParent(first.Info.DirectoryPath)!.Name);
            ...
            {
                Equal(7, diagnostics.RootElement.GetProperty("schemaVersion").GetInt32());
