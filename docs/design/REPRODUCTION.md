...
Settlement出生所属には二つの確定経路がある。両親が同じActive Settlement所属なら、受胎位置に関係なくそのSettlement IDをBirthRequestへ保存し、出生Cellは通常どおり受胎時の両親隣接Cellから選ぶ。片親だけが所属する場合、または両親の所属が異なる場合は、少なくとも一方の所属先で受胎時の両者が同じ一意なActive Settlement Core内にいる従来条件だけを認め、Settlement IDとCore内配置制約を保存する。複数候補が成立して一意に定まらない場合は保存しない。

Tick末に全Requestをまとめて解決する。各Requestは受胎時の両親隣接Cell和集合にある空きCellからseed付きで希望Cellを選ぶ。両親の同一所属でSettlement IDを保存したRequestは場所候補をCoreへ制限せず、出生した子へ同SettlementのMembershipThreshold相当AffinityとActive Affiliationを設定する。従来のCore条件で保存したRequestだけは候補を同Core内へ限定し、全Core候補が占有等で尽きた場合はCore外へ迂回せず出生失敗とする。どちらも出生解決時にSettlementがActiveでなくなっていれば通常の無所属出生へ戻す。
...
Settlement Affinity値、Founder状態、Invasion参加、Auraは遺伝対象ではない。条件付き出生所属はBirthRequestへ固定した社会状態から子のAffinityをMembershipThresholdへ新規設定するもので、親のAffinity値を複製しない。両親が同じActive Settlement所属する場合だけActive Affiliationを場所非依存で継承し、片親所属・異所属は従来の一意なCore条件を必要とする。Settlement Core内のReproduction Successは、当事者へv0.2 ConfigのSettlement Affinityを加え、Hotspot生成判定の機械可読Eventとなる。

## Tests

非遺伝情報が子へ移らないこと、同一Settlement所属の両親ならCore外でも通常位置へ出生して同所属で開始すること、片親所属では一意なCore条件を外れると所属しないこと、各項目の独立blend、Mutation再現性とClamp、MarkがBase値を変更しないこと、Candidateが対象RealityのHP/Cooldownを読まないこと、Reality precondition検証、Reject時のIntentとNeed/Cooldown不変、Accept時最大1回のIntent置換、Birth位置競合のqueue順非依存と失敗時コスト維持をheadless testで検証する。
