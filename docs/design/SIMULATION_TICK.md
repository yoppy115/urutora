...
両親が同じActive Settlement所属なら、そのSettlement IDを場所に関係なくBirthRequestへ固定し、出生位置は通常の親近傍候補を使う。片親所属・異所属の一意なCore条件ではSettlement IDとCore配置要否を固定する。後の所属変更でこの判定を差し替えない。
