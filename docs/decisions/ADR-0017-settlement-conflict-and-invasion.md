...
Frictionは対称Settlement Pairの非負値、Hostilityは方向性stateとして分離する。初期v0.2のraw加算・段階decayはADR-0024の人口scale付き日次impulse / 指数decayに置換された。Counterattackによる二重加算とInvasion中Combat一件ごとの加算をしない境界は維持する。
