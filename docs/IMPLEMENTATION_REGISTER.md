...
| v0.2.6 Config / diagnostics schema | Implemented | `V0_2_6_FISSION_INVASION_THROUGHPUT.md`, `simulation/configs/v0-default.json`, `LOGGING.md` | Simulation Config schema 6 / `v0.2.6-default-1`、diagnostics schema 7。全Alive所属者動員、60日cooldown、攻撃者不在base / Center距離係数を型付き設定へ分離し、旧割合動員・re-arm設定を削除 |
| v0.2.6全所属者Invasion参加 | Implemented | `INVASION_V025.md`, `ADR-0031` | 開始時のAlive affiliated memberを全員Core / Frontier cohortへ分類し、当日Restも含める。Active Invasion stateが残る人物がいれば部分動員せず開始を保留する |
